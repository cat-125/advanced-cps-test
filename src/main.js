...
win.append((new cheatgui.Switch('Circles', true)).bind(config, 'circles'));
win.append((new cheatgui.Switch('Large CPS text', true)).bind(config, 'cpsText'));
win.append((new cheatgui.Switch('Show info', true)).bind(config, 'infoText'));
...
document.addEventListener('pointerdown', e => {
	...
	if (config.circles) circles.push({ x: mouseX, y: mouseY, life: 100 });
	isMouseDown = true;
	clicks++;
	const t = getTime();
	delaySum += t - prevTime;
